...
bool HelloWorld::init()
{
    ...
    CCTextureCache::sharedTextureCache()->addImage("Agent.png");
    ...
    label = LabelTTF::create(LocalizedString::create("HELLOWORLD")->getCString(),
                             "Arial", 24);
    ...
    worldMap->setPosition(Vec2(visibleSize.width / 2 + origin.x,
                               visibleSize.height / 2 + origin.y));

    backgroundHotSpot = new Image();
    backgroundHotSpot->initWithImageFile("WorldMapR0.png");
    int x = 3;
    if (backgroundHotSpot->hasAlpha())
        x = 4;
    ...
    mario->cocos2d::Node::setPosition(newLevel->positionPlayer.x,
                                      newLevel->positionPlayer.y);
    ...
    cocos2d::MenuItem* pPauseItem = cocos2d::MenuItemImage::create(
        "PauseNormal.png", "PauseSelected.png", this,
        menu_selector(HelloWorld::menuPauseCallback));

    pPauseItem->setPosition(pCloseItem->getPositionX() - 120 * 3,
    ...
    cocos2d::MenuItem* pPlayItem = cocos2d::MenuItemImage::create(
        "PlayNormal.png", "PlaySelected.png", this,
        menu_selector(HelloWorld::menuPlayCallback));

    pPlayItem->setPosition(pCloseItem->getPositionX() - 80 * 3,
                           pCloseItem->getPositionY());
    ...
    cocos2d::MenuItem* pFastForwardItem = cocos2d::MenuItemImage::create(
        "FastForwardNormal.png", "FastForwardSelected.png", this,
        menu_selector(HelloWorld::menuFastForwardCallback));

    pFastForwardItem->setPosition(pCloseItem->getPositionX() - 40 * 3,
                                  pCloseItem->getPositionY());
    ...
    pMenu->setPosition(
        Point(cocos2d::Director::getInstance()->getWinSize().width - pCloseItem->getContentSize().width,
    ...
    // cocos2d::Node *pNode =
    // cocostudio::SceneReader::getInstance()->createNodeWithSceneFile("MainMenu_1.json");
    // add the node to new scene
    // this->addChild(pNode,0,1);

    setBackgroundSize();

    points = DrawPoint::create();
    worldMap->addChild(points);

    // CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("scene2track.mp3",
    // true);
                                ...
                                cocos2d::Event* event)
{
    ...
    if (touches.size() == 2) {
        ...
        /*cocos2d::Point touch0 = touches.at(0)->getLocation();
         ...
         cocos2d::Point pointBetweenFingers =
         ccpAdd(touch0,(ccpSub(touch1,touch0)/2));
         Point visible = CCDirector::sharedDirector()->getVisibleSize();
         Point currentCenter = worldMap->getPosition() - ( pointBetweenFingers -
         visible/2 ) ;
...
bool HelloWorld::selectSpriteForTouch(cocos2d::Sprite* sprite,
                                      cocos2d::Point touchLocation)
{
...
void HelloWorld::setBackgroundWidth(int width) { backgroundWidth = width; }

void HelloWorld::setBackgroundHeight(int height) { backgroundHeight = height; }
...
void HelloWorld::menuCloseCallback(Ref* pSender)
{
    ...
    CCUserDefault::getInstance()->setIntegerForKey(
...
{
    newLevel->velocityMultiplier = 8;
}

void HelloWorld::update(float delta)
{
    // auto action = MoveTo::create(0.2,cocos2d::Point(newLevel->positionPlayer.x,
    // newLevel->positionPlayer.y));
    ...
    worldMap->convertToWorldSpace(p);
    points->appendPoint(p.x - 10, p.y, 1, 0, 0);
    points->scale = zoomScale;
    points->draw(cocos2d::Director::getInstance()->getRenderer(), worldMap->getNodeToWorldTransform(), false);*/
    Sprite* agent = Sprite::create("Agent.png");
    agent->setPosition(newLevel->positionPlayer.x - 10, newLevel->positionPlayer.y);
    worldMap->addChild(agent);
