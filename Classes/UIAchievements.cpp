...
bool UIAchievements::init()
{
    ...
    for (int i = 0; i < achs.size(); i++) {
        for (int j = 0; j < achs.at(i).size(); j++)
        {
            ...
            auto title = Label::createWithTTF(LocalizedString::create(string("TITLE_LVL"+key).c_str(), "achievements"), "fonts/BebasNeue.otf", 47 * GameData::getInstance()->getRaConversion());
            ...
            auto text = Label::createWithTTF(LocalizedString::create(string("DESCR_LVL"+key).c_str(), "achievements"), "fonts/arial_rounded_mt_bold.ttf", 30 * GameData::getInstance()->getRaConversion());
