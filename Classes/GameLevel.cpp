...
void GameLevel::playLevel(void)
{
    ...
    while (_finishedGame == Running)
    {
        ...
        if (Timing::getInstance()->act == true) {
            ...
            {
                CCLOG("type %d: popsize: %d wood %d, mineral %d", i, int(_agents.at(i).size()), Agent::_resourcesPool.at(i).at(0), Agent::_resourcesPool.at(i).at(1));
            }
        }
    }
...
void GameLevel::generateInitialAgents(int type)
{
    ...
    while (i < numInitialAgents.at(type))
    {
        ...
        a->setLife(cocos2d::RandomHelper::random_int(0,175));
        a->setType(type);
