...
void GameLevel::dieAndReproduce(void)
{
    ...
    for (int i = dieAgentsSize - 1; i >= 0; i--) {
        ...
        if (power2Active > 0) {
            if (gameplayMap->isInBoostResistanceArea(agents.at(i)->getPosition()->getX() * float(2048 / 200),
                                                     agents.at(i)->getPosition()->getY() * float(1536 / 200))) {
                harm /= 2;
            }
        }
        ...
        //si viu, reproduce
            ...
            if (agents.size() < 3000) {
                ...
                int mobility = agents.at(i)->getValOfAttribute("att2");
                if (probReproduction > 0) {
                    int reproduce;
                    if (probReproduction >= 10) {
                        reproduce = 0;
