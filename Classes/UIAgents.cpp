...
bool UIAgents::init()
{
    ...
    for (map<string, int>::const_iterator it = atts.begin(); it != atts.end(); it++) {
        ...
        if (GameLevel::getInstance()->getAttributeCost(it->first) != 0) {
    ...
    for (int i = 0; i < keys.size(); i++) {
        auto attLabel = Label::createWithSystemFont(string(LocalizedString::create(keys.at(i).c_str())->getCString())
                                                    + " (" + to_string(GameLevel::getInstance()->getAttributeCost(keys.at(i))) + ")",
                                                    "Arial", 80);
        ...
        for (int j = 0; j < 10; j++) {
            ...
            if (j >= GameLevel::getInstance()->getAgentAttribute(keys.at(i))) {
                blankAttribute->setPosition(Vec2(posX, posY));
                this->addChild(blankAttribute, 1, tag);
            } else if (j < GameLevel::getInstance()->getAgentAttribute(keys.at(i))) {
...
void UIAgents::minusAttCallback(Ref* pSender)
{
    ...
    if (GameLevel::getInstance()->getAgentAttribute(keys.at(i)) > 0) {
        GameLevel::getInstance()->setAgentAttribute(keys.at(i), GameLevel::getInstance()->getAgentAttribute(keys.at(i)) - 1);
        GameLevel::getInstance()->setAttributeCost(keys.at(i), GameLevel::getInstance()->getAttributeCost(keys.at(i)) + 1);
        Label* l = (Label*)this->getChildByTag((i + 1) * 1000);
        l->setString(string(LocalizedString::create(keys.at(i).c_str())->getCString())
                     + " (" + to_string(GameLevel::getInstance()->getAttributeCost(keys.at(i))) + ")");

        auto filledAttribute = this->getChildByTag(GameLevel::getInstance()->getAgentAttribute(keys.at(i)) + (i * 10));
        auto blankAttribute = Sprite::create("BlankAttributePointButton.png");
        blankAttribute->setPosition(filledAttribute->getPosition());
        this->removeChildByTag(GameLevel::getInstance()->getAgentAttribute(keys.at(i)) + (i * 10));
        this->addChild(blankAttribute, 1, GameLevel::getInstance()->getAgentAttribute(keys.at(i)) + (i * 10));
    }
}
...
void UIAgents::plusAttCallback(Ref* pSender)
{
    ...
    if (GameLevel::getInstance()->getAgentAttribute(keys.at(i)) < 10) {
        GameLevel::getInstance()->setAgentAttribute(keys.at(i), GameLevel::getInstance()->getAgentAttribute(keys.at(i)) + 1);
        GameLevel::getInstance()->setAttributeCost(keys.at(i), GameLevel::getInstance()->getAttributeCost(keys.at(i)) + 1);
        Label* l = (Label*)this->getChildByTag((i + 1) * 1000);
        l->setString(string(LocalizedString::create(keys.at(i).c_str())->getCString())
        ...
        auto blankAttribute = this->getChildByTag((GameLevel::getInstance()->getAgentAttribute(keys.at(i)) - 1) + (i * 10));
        auto filledAttribute = Sprite::create("FilledAttributePointButton.png");
        filledAttribute->setPosition(blankAttribute->getPosition());
        this->removeChildByTag((GameLevel::getInstance()->getAgentAttribute(keys.at(i)) - 1) + (i * 10));
        this->addChild(filledAttribute, 1, (GameLevel::getInstance()->getAgentAttribute(keys.at(i)) - 1) + (i * 10));
    }
}